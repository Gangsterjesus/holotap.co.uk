...
export function classifySeverity(event: string): Severity {
  if (event.includes("identity:anonymous")) return "info";
  if (event.includes("identity:qr")) return "low";
  if (event.includes("identity:session")) return "medium";
  if (event.includes("identity:founder")) return "high";
  if (event.includes("payment:failure")) return "critical";
