...
export function classifySeverity(
  ...
  if (event.startsWith("identity:anonymous")) return "info";
  if (event.startsWith("identity:qr")) return "low";
  if (event.startsWith("identity:session")) return "medium";
  if (event.startsWith("identity:founder")) return "high";

  // Payment lifecycle events (Flow‑8)
  if (event.startsWith("payment:failure")) return "critical";
  if (event.startsWith("payment:settlement")) return "medium";
  if (event.startsWith("payment:initiation")) return "low";

  // System events (Flow‑12)
  if (event.startsWith("system:")) return "info";
